...
{
    public class AddNewProductRequestValidator : AbstractValidator<AddNewProductRequestDto>
    {
        public AddNewProductRequestValidator()
        {
            ...
            RuleFor(x => x.Description).NotNull().WithMessage(string.Format(ErrorMessage.RequierdMsg, "توضیحات محصول"));
            RuleFor(x => x.Invertory).NotNull().WithMessage(string.Format(ErrorMessage.RequierdMsg, "موجودی انبار محصول"));
            RuleFor(x => x.Images).NotEmpty().WithMessage(string.Format(ErrorMessage.RequierdMsg, "تصاویر"));
            RuleFor(x => x.Features).NotEmpty().WithMessage(string.Format(ErrorMessage.RequierdMsg, "ویژگی ها"));
        }
    }
}
